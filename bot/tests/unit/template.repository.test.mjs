...
test('renderTemplateContent replaces known placeholders and trims whitespace', () => {
  const template = 'Hi {{name}}, {{eventTitle}} is coming up on {{eventDate}}. {{eventTimeLine}} {{registrationLine}} Hope you can make it.'
  const rendered = renderTemplateContent(template, {
    name: 'Ada',
    eventTitle: 'Youth Summit',
    eventDate: 'June 2, 2026',
    eventTimeLine: 'It starts at 09:30.',
    registrationLine: 'Register here: https://example.com/register'
  })

  assert.equal(
    rendered,
    'Hi Ada, Youth Summit is coming up on June 2, 2026. It starts at 09:30. Register here: https://example.com/register Hope you can make it.'
  )
})
